...
export default function Chat() {
    ...
    const detectSpinRequest = (message: string) => {
        ...
        const spinKeywords = [
            'spin', 'wheel', 'mystery box', 'random',
            'turn', 'roll', 'try my luck', 'chance', 'draw', 'play'
        ];
        const claimKeywords = ['claim', 'faucet', 'reward', 'prize', 'win', 'get', 'receive'];

        const hasSpinKeyword = spinKeywords.some(keyword => lowerCaseMessage.includes(keyword));
        const hasClaimKeyword = claimKeywords.some(keyword => lowerCaseMessage.includes(keyword));

        // Trigger if it has a spin keyword, or a claim keyword that isn't just a simple check.
        if (hasSpinKeyword) {
            return true;
        }
        if (hasClaimKeyword) {
            // Avoid triggering on questions like "when can I claim?"
            if (lowerCaseMessage.startsWith("when") || lowerCaseMessage.startsWith("what") || lowerCaseMessage.startsWith("how")) {
                return false;
            }
            return true;
        }
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {messages.length === 0 && !error ? (
                        <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <h2 className="text-2xl font-bold mb-2 text-black dark:text-white/90">Ready to spin the Mystery Box?</h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg dark:text-gray-400">
                                <Button variant="outline" className="justify-start text-left p-4 h-auto" onClick={() => 
                                    handleInputChange({ target: { value: "Spin the Mystery Box wheel!" } } as any)}>
                                    🎰 Spin the Mystery Box
                                </Button>
                                <Button variant="outline" className="justify-start text-left p-4 h-auto" onClick={() => 
                                    handleInputChange({ target: { value: "How does the Mystery Box faucet work?" } } as any)}>
                                    📦 How it works
                                </Button>
                                <Button variant="outline" className="justify-start text-left p-4 h-auto" onClick={() => 
